''' Class and functions to get data of CIFAR10 for the task of semi-supervised learning '''
...
def get_cifar10(root, num_labeled, num_expand_x, num_expand_u):
    '''
    inputs  :
        - root         : path to original CIFAR10 dataset (string)
        - num_labeled  : number of labeled data to use (integer)
        - num_expand_x : number of labeled data to expand with simple transformation
        - num_expand_u : number of unlabeled data to expand with "weak" and "strong" strategy
    
    outputs :
        - train_labeled_dataset   : train labeled dataset (list of tuples)
        - train_unlabeled_dataset : train unlabeled dataset (list of tuples)
        - test_dataset            : test dataset (list of tuples)
    '''
    ...
    train_labeled_idxs, train_unlabeled_idxs = x_u_split(base_dataset.targets, num_labeled, num_expand_x, num_expand_u, num_classes=10)

    train_labeled_dataset = CIFAR10SSL(root, train_labeled_idxs, train=True, transform=transform_labeled)

    train_unlabeled_dataset = CIFAR10SSL(root, train_unlabeled_idxs, train=True, transform=TransformFix(mean=cifar10_mean, std=cifar10_std))
...
def x_u_split(labels, num_labeled, num_expand_x, num_expand_u, num_classes):
    '''
    inputs :
        - labels       : list of label for each image of the dataset (integer)
        - num_labeled  : number of labeled data to use (integer)
        - num_expand_x : number of 
        - num_expand_u : number of
        - num_classes  : number of classes in the dataset
    
    outputs :
        - labeled_idx   : index of labeled data
        - unlabeled_idx : index of unlabeled data
    ...
    labeled_idx       = np.hstack([labeled_idx for _ in range(exapand_labeled)])
...
class TransformFix(object):
    '''
    Class for generating "strong" and "weak" version of an image 
    '''
